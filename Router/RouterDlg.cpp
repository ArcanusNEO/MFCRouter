...
BOOL CRouterDlg::OnInitDialog() {
  ...
  */
  int flag = pcap_findalldevs_ex(
    PCAP_SRC_IF_STRING, NULL, &alldevs, errbuf);  //获取设备列表
  if (flag == -1)                                 //错误处理
  {
  ...
  //输出用户交互指示信息
  log_ctrl.SetWindowText(L"设备列表已获取，请双击打开目标网卡开始路由！\r\n");
...
CString convert_sockaddr_to_str(sockaddr* addr) {
  if (addr == NULL) return L"";
  else {
    CString str;
    unsigned char* temp = (unsigned char*) addr->sa_data;
    str.Format(L"%d.%d.%d.%d", temp[2], temp[3], temp[4], temp[5]);
    return str;
  }
  return CString();
}
...
CString convert_IPaddr_to_str(DWORD addr) {
  ...
  str.Format(L"%d.%d.%d.%d", temp[0], temp[1], temp[2], temp[3]);
  return str;
}

// 将MAC addr中地址转换为字符串返回
CString convert_MAC_to_str(BYTE* MAC) {
  if (MAC == NULL) return L"";
  else {
    //格式化MAC地址数据
    CString hostmac;
    hostmac.Format(L"%02x:%02x:%02x:%02x:%02x:%02x", MAC[0], MAC[1], MAC[2],
      MAC[3], MAC[4], MAC[5]);
    return hostmac;
  }
  return CString();
...
bool compMAC(BYTE* m1, BYTE* m2) {
  for (int i = 0; i < 6; i++) {
    if (int(m1[i]) != int(m2[i])) return false;
  }
  return true;
}
...
bool sendICMP(
  ...
  // 填充帧首部
  memcpy(
    ((FrameHeader_t*) ICMPBuf)->DesMAC, ((FrameHeader_t*) pkt_data)->SrcMAC, 6);
  memcpy(
    ((FrameHeader_t*) ICMPBuf)->SrcMAC, ((FrameHeader_t*) pkt_data)->DesMAC, 6);
  ((FrameHeader_t*) ICMPBuf)->FrameType = htons(0x0800);

  // 填充IP首部
  ((IPHeader_t*) (ICMPBuf + 14))->Ver_HLen =
    ((IPHeader_t*) (pkt_data + 14))->Ver_HLen;
  ...
  ((IPHeader_t*) (ICMPBuf + 14))->Flag_Segment =
    ((IPHeader_t*) (pkt_data + 14))->Flag_Segment;
  ((IPHeader_t*) (ICMPBuf + 14))->TTL = 64;
  ((IPHeader_t*) (ICMPBuf + 14))->Protocol = 1;
  ((IPHeader_t*) (ICMPBuf + 14))->SrcIP =
    Dlg->IP_addr[0].IP;  //((IPHeader_t *)(pkt_data+14))->DstIP;
  ((IPHeader_t*) (ICMPBuf + 14))->DstIP =
    ((IPHeader_t*) (pkt_data + 14))->SrcIP;
  ((IPHeader_t*) (ICMPBuf + 14))->Checksum = 0;
  ((IPHeader_t*) (ICMPBuf + 14))->Checksum =
    ChecksumCompute((unsigned short*) (ICMPBuf + 14), 20);
  ...
  ((ICMPHeader_t*) (ICMPBuf + 34))->Checksum =
    ChecksumCompute((unsigned short*) (ICMPBuf + 34), 36);
...
UINT RecvARP(PVOID hwnd) {
  ...
  while (
    !Dlg
       ->stop_ARP_Catch_Thread)  //循环捕获网络数据包，通过参数stop_thread 控制停止捕获
  {
    //捕获数据包的主体代码，利用pcap_next_ex函数执行
    int flag = 0;
    TRY {
      flag =
        pcap_next_ex(Dlg->opened_pcap,  //pcap_t指针，说明捕获那个网卡上的数据包
    ...
    CATCH(CException, e) {
      ...
      Dlg->MessageBox(L"捕获异常" + (CString) error);
      memset(error, 0, 1024);
      continue;
    }
    END_CATCH;
    ...
    if (flag == 1) {
      ...
      if (int(ntohs(FHeader->FrameType)) == int(0x0806))  //接收到ARP报文
      {
        ...
        if (int(ntohs(ARP->ProtocolType)) == int(0x0800)) {
          ...
          {
            if (
              (ARP->SendIP == Dlg->IP_addr[0].IP ||
                ARP->SendIP ==
                  Dlg->IP_addr[1]
                    .IP))  //如果获取的ARP响应来自本机，则可通过该ARP响应获取本机的MAC地址
            {
              if (Dlg->host_MAC != NULL)
                continue;  //如果已经接收过了，则放弃该报文
              //assert(!Dlg->host_MAC);
              Dlg->host_MAC = new BYTE[6];
              CopyMAC(ARP->SendHa,
                Dlg
                  ->host_MAC);  //获取本机的MAC地址，在日志中进行输出，并启动相应的环境
              CString log_t;
              Dlg->log_ctrl.GetWindowText(log_t);
              Dlg->log_ctrl.SetWindowText(log_t + L">>网卡MAC地址：" +
                convert_MAC_to_str(Dlg->host_MAC) + L"\r\n");
              Dlg->PostMessage(WM_BEGINROUTE, 0, 0);
            } else {
              //Dlg->MessageBox(L"接收到非本机发送的ARP报文");
              Dlg->ARP_pktdata_list.AddTail(pkt_data);  //将报文传入缓冲区
              Dlg->PostMessage(
                WM_ARPPACKET, 0, 0);  //发送报文接收成功消息触发ARP报文解析函数
            }
          }
        }
      }
  ...
  CString log_t;
  Dlg->log_ctrl.GetWindowText(log_t);
  Dlg->log_ctrl.SetWindowText(
    log_t + L">>捕获本地ARP数据包线程终止！" + L"\r\n");
  return 0;
}
...
UINT RecvRouteP(PVOID hwnd) {
  ...
  while (
    !Dlg
       ->stop_Route_Catch_Thread)  //循环捕获网络数据包，通过参数stop_thread 控制停止捕获
  {
    //捕获数据包的主体代码，利用pcap_next_ex函数执行
    int flag = 0;
    TRY {
    ...
    if (flag == 1) {
      ...
      if (int(ntohs(FHeader->FrameType)) == int(0x0800))  //接收到IPv4报文
      {
        ...
        Dlg->PostMessage(
          WM_ROUTEPACKET, 0, 0);  //发送报文接收成功消息触发报文解析函数
      }
      if (int(ntohs(FHeader->FrameType)) == int(0x0806))  //接收到ARP报文
      {
        Dlg->ARP_pktdata_list.AddTail(pkt_data);  //将报文传入缓冲区
  ...
  CString log_t;
  Dlg->log_ctrl.GetWindowText(log_t);
  Dlg->log_ctrl.SetWindowText(log_t + L">>捕获路由数据包线程终止！" + L"\r\n");
  return 0;
}
...
LRESULT CRouterDlg::OnROUTEPacket(WPARAM wParam, LPARAM lParam) {
  TRY {
    ...
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t +
      L"=========================================================\r\n" +
      L">>接收可路由报文:" + convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) +
      L" -- " + convert_IPaddr_to_str(IP_Data->IPHeader.DstIP) + L"  " +
      convert_MAC_to_str(IP_Data->FrameHeader.SrcMAC) + L" -- " +
      convert_MAC_to_str(IP_Data->FrameHeader.DesMAC) + L"\r\n");
    ...
    IP_Data->IPHeader.Checksum =
      htons(IPHeader_ckeck((WORD*) &IP_Data->IPHeader));  //重新计算校验和
    //数据包超时，转入ICMP报文处理流程
    CString t;
    t.Format(L"%d", IP_Data->IPHeader.TTL);
    log_ctrl.SetWindowText(log_t + L"--->>TTL:" + t + L"\r\n");
    ...
    if (int(IP_Data->IPHeader.TTL) <= 0) {
      ...
      if (sendICMP(opened_pcap, 11, 0, pkt_data)) {
        ...
        log_ctrl.GetWindowText(log_t);
        log_ctrl.SetWindowText(log_t +
          L"--->>成功向源地址发送ICMP超时报文: 源IP地址:" +
          convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L"\r\n");
      } else {
        //发送失败
        log_ctrl.GetWindowText(log_t);
        log_ctrl.SetWindowText(log_t +
          L"--->>ERROR:向源地址发送ICMP超时报文失败: 源IP地址:" +
          convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L"\r\n");
      }
      return LRESULT();
    }

    if (IPHeader_ckeck((WORD*) (&IP_Data->IPHeader)) !=
      ntohs(IP_Data->IPHeader.Checksum)) {
      //日志信息输出
      CString checksum;
      checksum.Format(L"报文校验和:0x%04x,计算校验和:0x%04x",
        IP_Data->IPHeader.Checksum,
        IPHeader_ckeck((WORD*) (&IP_Data->IPHeader)));  //输出格式化
      log_ctrl.GetWindowText(log_t);
      log_ctrl.SetWindowText(
        log_t + L"--->>ERROR:校验和错误！\r\n======>" + checksum + L"\r\n");
      //ICMP处理
      pkthdr_list.RemoveHead();  //移除解析完的头部数据包
      pktdata_list.RemoveHead();
    ...
    while (pos != NULL) {
      ...
      if (compIP(item->netMask & IP_Data->IPHeader.DstIP, item->destNet) &&
        int(item->netMask) >
          int(
            longestNetMask))  //匹配到相应的路由并且掩码长度大于之前的掩码长度，默认设置为0
      {
        find = true;
    ...
    if (!find)  //未匹配到路由
    {
      ...
      if (sendICMP(opened_pcap, 3, 0, pkt_data)) {
        ...
        log_ctrl.GetWindowText(log_t);
        log_ctrl.SetWindowText(log_t +
          L"--->>成功向源地址发送ICMP网络不可达报文: 源IP地址:" +
          convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L"\r\n");
      } else {
        //发送失败
        log_ctrl.GetWindowText(log_t);
        log_ctrl.SetWindowText(log_t +
          L"--->>ERROR:向源地址发送ICMP网络不可达报文失败: 源IP地址:" +
          convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L"\r\n");
      }
      return LRESULT();
    }
    ...
    if (nextHop_IP == 0x0)  //直接路由
    {
      ...
      CString test;
      test.Format(L"0x%04x-->0x%04x", ntohs(IP_Data->IPHeader.Checksum),
        IPHeader_ckeck((WORD*) &IP_Data->IPHeader));
      log_ctrl.GetWindowText(log_t);
      log_ctrl.SetWindowText(
        log_t + L"--->>直接路由，重新计算校验和:" + test + L"\r\n");
    } else {
      //非直接路由
      CString test;
      test.Format(L"0x%04x-->0x%04x", ntohs(IP_Data->IPHeader.Checksum),
    ...
    if (getMACForIP(IP_Data->IPHeader.DstIP) == NULL) {
      ...
      BYTE* SendHa_t = NULL;
      for (int i = 0; i < 2; i++) {
        if (compIP(nextHop_IP & IP_addr[i].netMask,
              IP_addr[i].IP & IP_addr[i].netMask)) {
      ...
      //记录日志
      log_ctrl.GetWindowText(log_t);
      log_ctrl.SetWindowText(log_t + L"--->>发送ARP请求报文到 IP:" +
        convert_IPaddr_to_str(RecvIP_t) + L"\r\n");
      //缓存发送报文
      int ID = 0;
      if ((ID = getTimerID()) == 0) {
        //缓冲区满了
        //输出日志
        log_ctrl.GetWindowText(log_t);
        log_ctrl.SetWindowText(log_t + L">>发送缓冲区已满(15/15)，丢弃数据包:" +
          convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L" -- " +
          convert_IPaddr_to_str(IP_Data->IPHeader.DstIP) + L"  " +
          convert_MAC_to_str(IP_Data->FrameHeader.SrcMAC) + L" -- " +
          convert_MAC_to_str(IP_Data->FrameHeader.DesMAC) + L"\r\n");
      } else {
        ...
        //输出日志
        log_ctrl.GetWindowText(log_t);
        log_ctrl.SetWindowText(log_t + L">>缓存报文，等待接收ARP报文:" +
          convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L" -- " +
          convert_IPaddr_to_str(IP_Data->IPHeader.DstIP) + L"  " +
          convert_MAC_to_str(IP_Data->FrameHeader.SrcMAC) + L" -- " +
          convert_MAC_to_str(IP_Data->FrameHeader.DesMAC) + L"\r\n");
    ...
    //log_ctrl.GetWindowText(log_t);
    //log_ctrl.SetWindowText(log_t + L"--->>成功获取下一跳步IPMAC映射关系: IP:" + convert_IPaddr_to_str(nextHop_IP)
    //  +" -- MAC:"+convert_MAC_to_str(nextHop_MAC)+ L"\r\n");
    ...
    if (sendIPFrame(opened_pcap, (u_char*) IP_Data, totalLen)) {
      //成功转发
      log_ctrl.GetWindowText(log_t);
      log_ctrl.SetWindowText(log_t + L"--->>转发数据包成功！from:" +
        convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L" -- " + L"to:" +
        convert_IPaddr_to_str(IP_Data->IPHeader.DstIP) +
        L"\r\n next hop IP<->MAC:" + convert_IPaddr_to_str(nextHop_IP) +
        L" <--> " + convert_MAC_to_str(nextHop_MAC) + L"\r\n");
    } else {
      //未转发成功
      log_ctrl.GetWindowText(log_t);
      log_ctrl.SetWindowText(log_t + L"--->>ERROR:转发失败！from:" +
        convert_IPaddr_to_str(IP_Data->IPHeader.SrcIP) + L" -- " + L"to:" +
        convert_IPaddr_to_str(IP_Data->IPHeader.DstIP) +
        L"\r\n next hop IP<->MAC:" + convert_IPaddr_to_str(nextHop_IP) +
        L" <--> " + convert_MAC_to_str(nextHop_MAC) + L"\r\n");
  ...
  CATCH(CException, e) {
    TCHAR error[1024](L"\0");
    e->GetErrorMessage(error, 1024);
    MessageBox(L"处理异常" + (CString) error);
    memset(error, 0, 1024);
  }
  END_CATCH;
...
LRESULT CRouterDlg::OnARPPacket(WPARAM wParam, LPARAM lParam) {
  TRY {
    ...
    //  log_ctrl.GetWindowText(log_t);
    //  log_ctrl.SetWindowText(log_t + L">>IP-MAC:" +
    //    L"IP:" + convert_IPaddr_to_str(t->IP) +  " <--> " +
    //    L"MAC:" + convert_MAC_to_str(t->MAC) + L"\r\n");
    //  IP_MAC.GetNext(pos);
    //}
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t + L">>捕获ARP报文解析成功:" + L"IP:" +
      convert_IPaddr_to_str(ARP->SendIP) + L" <--> " + L"MAC:" +
      convert_MAC_to_str(ARP->SendHa) + L"\r\n");
      ...
      while (time--) {
        ...
        while (pos != NULL) {
          ...
          if (compIP(item->TargetIP, ARP->SendIP))  //匹配可以发送的报文
          {
            //修改帧头部信息并取得帧头部长度
            CopyMAC(
              ARP->SendHa, ((Data_t*) (item->PktData))->FrameHeader.DesMAC);
            ...
            if (sendIPFrame(opened_pcap, (u_char*) item->PktData, item->len)) {
              ...
              log_ctrl.SetWindowText(log_t +
                L"--->>转发缓冲区数据包成功！from:" +
                convert_IPaddr_to_str(
                  ((Data_t*) (item->PktData))->IPHeader.SrcIP) +
                L" -- " + L"to:" +
                convert_IPaddr_to_str(
                  ((Data_t*) (item->PktData))->IPHeader.DstIP) +
                L"\r\n next hop IP<->MAC:" +
                convert_IPaddr_to_str(item->TargetIP) + L" <--> " +
                convert_MAC_to_str(ARP->SendHa) + L"\r\n");
            } else {
              //未转发成功
              log_ctrl.GetWindowText(log_t);
              log_ctrl.SetWindowText(log_t +
                L"--->>ERROR:转发缓冲区数据包失败，丢弃数据包！from:" +
                convert_IPaddr_to_str(
                  ((Data_t*) (item->PktData))->IPHeader.SrcIP) +
                L" -- " + L"to:" +
                convert_IPaddr_to_str(
...
LRESULT CRouterDlg::OnBeginRoute(WPARAM wParam, LPARAM lParam) {
  CString log_t;
  log_ctrl.GetWindowText(log_t);
  log_ctrl.SetWindowText(log_t + L">>开始初始化路由表！" + L"\r\n");
  //初始化缓冲区
  ...
  log_ctrl.GetWindowText(log_t);
  log_ctrl.SetWindowText(log_t + L">>完成路由表初始化！" + L"\r\n");

  // 设置过滤规则:仅仅接收arp响应帧和需要路由的帧

  // 设置过滤规则:仅仅接收arp响应帧和需要路由的帧
  CString Filter = L"arp and ether[21] == 0x02";
  struct bpf_program fcode;
  Filter.Format(
    L"(ether dst %s) and ((arp and (ether[21] == 0x02)) or (not((ip dst host %s) or (ip dst host %s))))",
    convert_MAC_to_str(host_MAC), convert_IPaddr_to_str(IP_addr[0].IP),
    convert_IPaddr_to_str(IP_addr[1].IP));
  USES_CONVERSION;
  char* FilterA = W2A(Filter);
  if ((pcap_compile(opened_pcap, &fcode, FilterA, 1,
         IP_addr[0].netMask) < 0) ||
    (pcap_setfilter(opened_pcap, &fcode) < 0)) {
    MessageBox(Filter +
      L"过滤规则编译不成功，请检查书写的规则语法是否正确！设置过滤器错误！");
    //return LRESULT();
  }
  //MessageBox(Filter);
  //开启路由数据包捕获线程
  stop_Route_Catch_Thread =
    false;  //创建新的线程之前，需要先将原来的线程关闭,并将线程控制参数stop_thread置位为0
  ...
  log_ctrl.GetWindowText(log_t);
  log_ctrl.SetWindowText(log_t + L">>成功创建数据包捕获线程！" + L"\r\n" +
    L">>>>>>>>>>>>>>>>>>>>>>> 开始路由 <<<<<<<<<<<<<<<<<<<<<<<" + L"\r\n");
...
void CRouterDlg::OnLbnDblclkList2() {
  ...
  if (stop_ARP_Catch_Thread == false) {
    ...
    CString log_t;
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t + L">>正在关闭ARP报文捕获线程！" + L"\r\n");
  }
  if (stop_Route_Catch_Thread == false) {
    stop_Route_Catch_Thread = true;
    CString log_t;
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t + L">>正在关闭路由报文捕获线程！" + L"\r\n");
  }
  ...
  for (d = alldevs; d != NULL;
       d = d->next)  //通过选中的接口名获得设备接口的地址，赋值给了类成员d，
    if ((CString) (d->name) ==
      sitem)  //之后可以通过d获取该接口信息，并将设备接口描述和各类地址显示
      break;
  ...
  CString log_t;
  log_ctrl.GetWindowText(log_t);
  log_ctrl.SetWindowText(log_t + L">>选择网卡：" + sitem + L"\r\n");
  ...
  for (a = d->addresses; a != NULL; a = a->next) {
    ...
    {
      CString ip_str;
      ip_str.Format(L">>IP地址%d：%s  子网掩码%d：%s", IPCount,
        convert_sockaddr_to_str(a->addr), IPCount,
        convert_sockaddr_to_str(a->netmask));
      //输出日志信息
      CString log_t;
      log_ctrl.GetWindowText(log_t);
      log_ctrl.SetWindowText(log_t + ip_str + L"\r\n");
      IP_addr[IPCount].IP = GetAddr_IP(a->addr);  //记录网卡的两个IP地址
      IP_addr[IPCount].netMask =
        GetAddr_IP(a->netmask);  //记录两个网卡的子网掩码
      IPCount++;                 //记录加1
      if (IPCount == 2)
        break;  //获得两个IP立即退出，当前的a保存网卡的第二个IP地址
    }
  }
  ...
  if (IPCount < 2)  //该网卡不存在两个IP地址报告错误退出函数
  {
    ...
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t +
      L">>ERROR:该网卡不具备两个IP地址，无法作为路由设备，请重新选择！" +
      L"\r\n\
====================================================\r\n");
    return;
  }
  ...
  //打开选择的设备
  //打开设备接口
  opened_pcap = pcap_open(
    d->name,  //设备接口名，直接从类成员d指针中获取，一旦选择之后，d指针指向选择的设备接口
    4096,     //获取数据包的最大长度
  ...
  if (opened_pcap == NULL)  //错误处理
    ...
    CString log_t;
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t + L">>成功创建ARP报文捕获线程！" + L"\r\n");
  }
  //开启ARP数据包捕获线程

  //通过对本机发送一个ARP请求获取本机的IPMAC映射关系
  BYTE SendHa_t[6] {
    0x66, 0x66, 0x66, 0x66, 0x66, 0x66};  //虚假源MAC地址 66-66-66-66-66-66
  ...
  {  //失败，退出函数
    CString log_t;
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t + L">>ERROR:发送ARP请求到本机失败！" +
      L"\r\n\
====================================================\r\n");
    return;
  } else {
    CString log_t;
    log_ctrl.GetWindowText(log_t);
    log_ctrl.SetWindowText(log_t + L">>成功发送ARP请求到本机！" + L"\r\n");
  }
}
...
void updateRouteTable() {
  ...
  while (pos != NULL) {
    ...
    if ((int(item->destNet) ==
            int(Dlg->IP_addr[0].IP & Dlg->IP_addr[0].netMask) &&
          int(item->netMask) == int(Dlg->IP_addr[0].netMask)) ||
      (int(item->destNet) ==
          int(Dlg->IP_addr[1].IP & Dlg->IP_addr[1].netMask) &&
        int(item->netMask) == int(Dlg->IP_addr[1].netMask)))
      continue;
    //将路由输出
    CString rt;
    rt.Format(L"%s -- %s -- %s", convert_IPaddr_to_str(item->netMask),
...
void CRouterDlg::OnBnClickedAdd() {
  ...
  if (!compIP(des_net & netMask, des_net)) {
    MessageBox(L"添加的路由错误！", L"ERROR", MB_OKCANCEL | MB_ICONERROR);
    return;
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (int(IP_addr[i].netMask & nexthop_ip) ==
      int(IP_addr[i].netMask & IP_addr[i].IP)) {
      InTheSameNet = true;
      break;
    }
  }
  if (!InTheSameNet) {
    MessageBox(
      L"添加的路由下一跳步无法抵达！", L"ERROR", MB_OKCANCEL | MB_ICONERROR);
    return;
  }
  ...
  while (pos != NULL) {
    ...
    if (int(item->destNet) == int(des_net) &&
      int(item->netMask) == int(netMask))  //路由关系已经存在
    {
      //判断修改的路由是否为自动生成的直接路由，不允许修改
      if ((int(item->destNet) == int(IP_addr[0].IP & IP_addr[0].netMask) &&
            int(item->netMask) == int(IP_addr[0].netMask)) ||
        (int(item->destNet) == int(IP_addr[1].IP & IP_addr[1].netMask) &&
          int(item->netMask) == int(IP_addr[1].netMask))) {
        MessageBox(L"试图修改直接路由，非法请求未通过！", L"ERROR",
          MB_OKCANCEL | MB_ICONERROR);
        return;
      }
      CString log;
      log.Format(
        L"添加路由已存在, 将修改为最新提交版本:\r\n\
>>> netmask:%s dest net:%s nexthop: %s(old)-->%s(new)\r\n",
        convert_IPaddr_to_str(netMask), convert_IPaddr_to_str(des_net),
        convert_IPaddr_to_str(item->nextHops),
        convert_IPaddr_to_str(nexthop_ip));
      item->nextHops = nexthop_ip;  //修改为最新的路由跳步
      CString log_t;                //记录日志信息
      log_ctrl.GetWindowText(log_t);
      log_ctrl.SetWindowText(log_t + log);
      MessageBox(L"添加路由已存在,将修改为最新提交版本！", L"提示",
  ...
  CString rt;
  rt.Format(L"%s -- %s -- %s ", convert_IPaddr_to_str(routeItem->netMask),
    convert_IPaddr_to_str(routeItem->destNet),
    convert_IPaddr_to_str(routeItem->nextHops));
  router_table_ctrl.InsertString(router_table_ctrl.GetCount(), rt);
  CString log;
  log.Format(L"添加路由:netmask:%s dest net:%s nexthop: %s\r\n",
    convert_IPaddr_to_str(netMask), convert_IPaddr_to_str(des_net),
    convert_IPaddr_to_str(nexthop_ip));
  CString log_t;  //记录日志信息
  log_ctrl.GetWindowText(log_t);
...
void CRouterDlg::OnBnClickedDel() {
  ...
  while (pos != NULL) {
      ...
      int(item->netMask) == int(netMask))  //路由关系已经存在
    {
      ...
      if ((int(item->destNet) == int(IP_addr[0].IP & IP_addr[0].netMask) &&
        ...
        (int(item->destNet) == int(IP_addr[1].IP & IP_addr[1].netMask) &&
          int(item->netMask) == int(IP_addr[1].netMask))) {
        MessageBox(L"试图删除直接路由，非法请求未通过！", L"ERROR",
          MB_OKCANCEL | MB_ICONERROR);
        return;
      }
      RouteTable.RemoveAt(pos);
      CString log;
      log.Format(L"删除路由成功: netmask:%s dest net:%s nexthop: %s\r\n",
        convert_IPaddr_to_str(netMask), convert_IPaddr_to_str(des_net),
        convert_IPaddr_to_str(item->nextHops));
      CString log_t;  //记录日志信息
      log_ctrl.GetWindowText(log_t);
  ...
  MessageBox(L"删除的路由表项不存在！", L"ERROR", MB_OKCANCEL | MB_ICONERROR);
}
